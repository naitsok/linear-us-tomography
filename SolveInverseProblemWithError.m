...
% усреденение ошибки: сколько раз усредн€ть
averageNumber = 1;
...
errorTimes = GenerateTimesError(times, delta, averageNumber);

A = CalculateLinearEqMatrix(errorTimes, dimX, dimY, precision);

speedMatrixOutput = CalculateSpeedMatrix(A, errorTimes, alpha, dimX, dimY, minSpeed, maxSpeed);
...
%generate image name
saveImageName = strcat(saveImageDir, '\', num2str(delta, '%10.2e_delta'), num2str(averageNumber, '%_d_avNum'), '_', sensorConfig, '_');
